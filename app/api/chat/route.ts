  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: 'APIキーが設定されていません。VercelのEnvironment Variablesで GEMINI_API_KEY を設定してください。' },
        { status: 500 }
      );
    }
...
3. 過度な雑談に走らず、ユーザーの意思決定や整理を助ける姿勢を保ってください。
    ...
    // 確実に動作する安定版モデル (gemini-1.5-flash) を指定
    ...
    if (!response.ok) {
      ...
      const detailMsg = errorData?.error?.message || 'APIキーまたはリクエストが無効です。';
      return NextResponse.json(
        { error: `Gemini通信エラー: ${detailMsg}` },
